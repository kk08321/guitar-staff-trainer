# Guitar Staff Trainer

五線譜の音符とクラシックギターの指板上の位置を対応づけて練習できる、ブラウザ向けトレーニングアプリです。

## 概要

Guitar Staff Trainer は、クラシックギター初学者が五線譜を読み、対応する弦とフレットを指板上で見つける練習をするためのアプリです。

画面にはランダムな音符が表示されます。対応する弦とフレット位置を指板上でタップして回答します。正解すると次の音符に進み、練習を続けられます。

## アクセス方法
Azure Static Web Appsで配信しています：

https://calm-stone-0c5855300.7.azurestaticapps.net/

iPhone/iPadから上記URLにアクセスし、ブラウザ上でそのまま動作しますが、
下記手順にてWebアプリモードで起動することでより快適に利用できます。

https://support.apple.com/ja-jp/guide/iphone/iphea86e5236/ios

## 主な機能

- クラシックギター向けの五線譜クイズ
- 開放弦と 1-12 フレットに対応した指板 UI
- ギターの移調を考慮したト音記号の譜面表示
- 複数の練習範囲
  - Basic: 初学者向けの低めの音域
  - Standard: 6 弦開放から 1 弦 4 フレットまでの低ポジション
  - Wide: 12 フレットまでを含む広めの範囲
- 短期的な出題の偏りを抑えるシャッフル出題
- 正答率と連続正解数の記録
- レスポンシブ対応のブラウザ UI

## 技術スタック

- React
- Vite
- lucide-react

## ライセンスと第三者素材

このプロジェクト本体は [MIT License](./LICENSE) で公開しています。

このアプリは、外部の画像・音声・Web フォントファイルを同梱していません。UI アイコンは `lucide-react` を使用しています。

第三者 OSS のライセンス情報は [THIRD_PARTY_NOTICES.md](./THIRD_PARTY_NOTICES.md) を参照してください。Web デプロイ時にも同じ内容を `/THIRD_PARTY_NOTICES.md` として配信します。

## 開発を始める

依存関係をインストールします。

```bash
npm install
```

開発サーバーを起動します。

```bash
npm run dev
```

本番用にビルドします。

```bash
npm run build
```

本番ビルドをローカルでプレビューします。

```bash
npm run preview
```

## Azure Static Web Apps

このプロジェクトは、静的な Vite アプリとしてデプロイできます。

推奨されるビルド設定は次のとおりです。

```text
App location: /
Output location: dist
Build command: npm run build
```

`staticwebapp.config.json` にはナビゲーションフォールバックが含まれているため、シングルページアプリケーションとして配信できます。

## 補足

クラシックギターは、記譜より 1 オクターブ低く鳴ります。このアプリでは、譜面上には記譜音を表示しつつ、回答判定では指板上の実音に対応する音を使っています。
